...
import confidence, decode, embedders, geoformer, modules, omegaplm, utils
...
class OmegaFoldCycle(modules.OFModule):
    ...
    def forward(
        ...
        struct_node = node_repr[..., 0, :, :]
        struct_edge = edge_repr
        ...
        return ret, prev_dict, struct_node, struct_edge
...
class OmegaFold(modules.OFModule):
    ...
    def forward(
        ...
        for cycle_data in inputs:
            ...
            glm_node_repr, glm_edge_repr, node_repr, edge_repr = self.deep_sequence_embed(
                p_msa,
                p_msa_mask,
                fwd_cfg
            )
            ...
            # 初始态
            init_node = glm_node_repr
            init_edge_repr = glm_edge_repr
            embed1_node_repr = node_repr
            embed1_edge_repr = edge_repr
            ...
            # 循环阶段
            embed2_node_repr = node_repr
            embed2_edge_repr = edge_repr

            result, prev_dict, struct_node, struct_edge = self.omega_fold_cycle(
                fasta=fasta,
                mask=p_msa_mask,
                node_repr=node_repr,
                edge_repr=edge_repr,
            ...
            if predict_with_confidence:
                if confidence_overall > max_confidence:
                    ...
                    final_result = result
                    # saved_embed['init_node'] = glm_node_repr[..., 0, :, :]
                    # saved_embed['init_edge'] = glm_edge_repr
                    # saved_embed['embed_node'] = embed1_node_repr[..., 0, :, :].cpu().numpy().astype(np.float16)
                    # saved_embed['embed_edge'] = embed1_edge_repr.cpu().numpy().astype(np.float16)
                    # saved_embed['cycle_node'] = embed2_node_repr[..., 0, :, :].cpu().numpy().astype(np.float16)
                    # saved_embed['cycle_edge'] = embed2_edge_repr.cpu().numpy().astype(np.float16)
                    saved_embed['struct_node'] = struct_node.cpu().numpy().astype(np.float16)
                    saved_embed['struct_edge'] = struct_edge.cpu().numpy().astype(np.float16)

            else:
                final_result = result
            # break
        return final_result, saved_embed
    ...
    def deep_sequence_embed(
        ...
        return glm_node_repr, glm_edge_repr, node_repr, edge_repr
